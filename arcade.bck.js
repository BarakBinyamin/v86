...
function drawScreen(t) {
  ...
  ctx.fillStyle = `rgba(0,255,150,${0.8 + Math.sin(t * 0.005) * 0.2})`;
  ctx.font = "28px monospace";
  ctx.fillText("ARCADE READY", 40, 200);
  // blinking cursor
  if (Math.floor(t / 500) % 2 === 0) {
    ctx.fillRect(300, 210, 10, 3);
  }
  // scanlines
  ...
  for (let i = 0; i < 50; i++) {
    ctx.fillStyle = `rgba(0,255,150,${Math.random() * 0.05})`;
    ctx.fillRect(Math.random() * w, Math.random() * h, 2, 2);
  }
}
function addCRT(){
  const canvas = document.createElement("canvas");
  canvas.id = "crt-screen";   // 👈 here
  canvas.width = 512;
  canvas.height = 512;
...
function addOverheadLight(x,y=3.5,z){
...
function main(){
  ...
  animate()       // the loop to get processing going
}
main()